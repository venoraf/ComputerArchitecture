...
void addEdge(char * x, char* y, int dist){
  ...
  while (loc < n->edgecount && strcmp((e+loc)->namepath,y) < 0)
    loc++;
  //printf("loc: %d, x: %s y: %s\n", loc, x,y);
  //move everything over
  //printf("%s, %s, %d\n", x, y, loc);
  if (n->edgecount > 0) {
    for (int i = n->edgecount; i >loc ; i--){
      //printf("moving i%d\n",i);
      strcpy((e+i)->namepath,(e+i-1)->namepath);
...
int main (int argc, char* argv[]) {
  ...
  // printf("%s\n", argv[2]);
  visited = malloc((sizeof(char)*16)*(totalNodes + 1));
  printed = malloc((sizeof(char)*16)*(totalNodes+1));
  //printf("malloc done\n");
  ...
  while(fgets(str, 50, pathFILE) != NULL){ 
    str[strcspn(str, "\n")] = '\0';
    memset(visited, '\0', 16*(totalNodes + 1));
    memset(printed, '\0', 16*(totalNodes + 1));
    queue = visited;
    breadthfirst(str);
    printf("%s\n", printed);
  }
